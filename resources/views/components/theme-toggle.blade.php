{{--
  Theme Toggle Button
  Uses Alpine + localStorage.
  Place anywhere inside a layout that already has Alpine running.
--}}
<div
    x-data="{
        theme: localStorage.getItem('theme') ?? 'system',
        get isDark() {
            return this.theme === 'dark' ||
                   (this.theme === 'system' && window.matchMedia('(prefers-color-scheme: dark)').matches);
        },
        set(t) {
            this.theme = t;
            localStorage.setItem('theme', t);
            this.apply();
        },
        apply() {
            const html = document.documentElement;
            html.classList.add('transitioning');
            if (this.isDark) {
                html.classList.add('dark');
            } else {
                html.classList.remove('dark');
            }
            setTimeout(() => html.classList.remove('transitioning'), 300);
        },
        init() {
            this.apply();
            window.matchMedia('(prefers-color-scheme: dark)').addEventListener('change', () => {
                if (this.theme === 'system') this.apply();
            });
        }
    }"
    class="relative"
    x-init="init()"
>
    {{-- Toggle button --}}
    <button
        x-on:click="set(isDark ? 'light' : 'dark')"
        class="flex items-center justify-center size-8 rounded-lg
               text-zinc-500 hover:text-zinc-800 dark:hover:text-zinc-200
               hover:bg-zinc-100 dark:hover:bg-white/8 transition"
        x-bind:title="isDark ? 'Switch to light mode' : 'Switch to dark mode'"
    >
        {{-- Sun (shown in dark mode) --}}
        <svg x-show="isDark" class="size-4" fill="none" viewBox="0 0 24 24"
             stroke="currentColor" stroke-width="1.75">
            <path stroke-linecap="round" stroke-linejoin="round"
                  d="M12 3v2.25m6.364.386-1.591 1.591M21 12h-2.25m-.386 6.364-1.591-1.591M12 18.75V21m-4.773-4.227-1.591 1.591M5.25 12H3m4.227-4.773L5.636 5.636M15.75 12a3.75 3.75 0 1 1-7.5 0 3.75 3.75 0 0 1 7.5 0Z"/>
        </svg>
        {{-- Moon (shown in light mode) --}}
        <svg x-show="!isDark" class="size-4" fill="none" viewBox="0 0 24 24"
             stroke="currentColor" stroke-width="1.75" style="display:none">
            <path stroke-linecap="round" stroke-linejoin="round"
                  d="M21.752 15.002A9.72 9.72 0 0 1 18 15.75c-5.385 0-9.75-4.365-9.75-9.75 0-1.33.266-2.597.748-3.752A9.753 9.753 0 0 0 3 11.25C3 16.635 7.365 21 12.75 21a9.753 9.753 0 0 0 9.002-5.998Z"/>
        </svg>
    </button>
</div>
